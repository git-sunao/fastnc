...
class halofit:
    def __init__(self, k, z, pklin, lgr=None):
        """
        k     (np.ndarray): array of comoving Fourier modes
        z     (np.ndarray): array of redshifts
        pklin (np.ndarray): table of linear power spectrum
        lgr   (np.ndarray): linear growth rate
        """
        self.k     = np.asarray(k)
        self.z     = np.asarray(z)
        self.pklin = np.atleast_2d(pklin).T
        self.lgr   = lgr
        if self.lgr is None:
            print('Assumes z[0] == 0, and k[0] is small enough so that lgr = pklin[0, :]/pklin[0, 0]')
            self.lgr = (self.pklin[0, :]/self.pklin[0, 0])**0.5
        assert self.k.size == self.pklin.shape[0], \
            'size of k bin ({}) does not match with shape of pk ({})'.format(self.k.size, self.pklin.shape)
        assert self.z.size == self.pklin.shape[1], \
            'size of z bin ({}) does not match with shape of pk ({})'.format(self.z.size, self.pklin.shape)
        assert self.lgr.size == self.z.size, \
            'size of lgr bin ({}) does not match with size of z ({})'.format(self.lgr.size, self.z.size)
    
    def set_cosmology(self, Om0, Ode0, ns, w0, wa, fnu0):
        """
        ...
        fnu0 : fraction of neutrino energy density relative to matter
        """
        self.cosmo = dict()
        self.cosmo['Om0'] = Om0
        ...
        self.cosmo['w0']  = w0
        self.cosmo['wa']  = wa
        self.cosmo['fnu0']= fnu0
        
    def _get_nearest_zidx(self, z):
        """
        Returns nearest index if redshift for a given redshift.
        """
        idx = np.abs(self.z - z).argmin()
        return idx
    
    def interpolate_pklin_at_zidx(self, k, zidx):
        """
        Interpolate the power spectrum for a given redshift index.
        """
        logpk = ius(np.log(self.k), np.log(self.pklin[:, zidx]))(np.log(k))
        pk = np.exp(logpk)
        return pk
    
    def get_interpolated_pklin(self, k, z):
        """
        k (np.array): array of comoving Fourier modes
        z (np.array): array of redshifts
        """
        # get the nearest z index
        # zidx = self._get_nearest_zidx(z)
        zidx = 0
        # get interpolated power spectrum
        pk = self.interpolate_pklin_at_zidx(k, 0)
        # rescale power spectrum by growth factor ratio at the reference redshift
        if self.z.size >= 2:
            pk *= (ius(self.z, self.lgr)(z)/self.lgr[0])**2
        return pk
    ...
    def prepare_lazy_arrays(self, zmid=0.5, dz_low=0.15, dz_high=0.3):
        # For acculate calcuration, we divide z array into two sections
    ...
    def get_bihalofit(self, k1, k2, k3, z, verbose=False, separate=False, all_physical=False):
        """
        k1, k2, k3 z
        """
        # N
